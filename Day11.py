...
class Monkey:
    ...
    def calculate_worry_level(self, item_value) -> int:
        self.inspected_items += 1
        worry_level = item_value ** self.potency * self.worry_mult + self.worry_add
        return int(worry_level)

    def worry_level_after_inspection(self, item) -> int:
        worry_level = int(self.calculate_worry_level(item)/3)
        return worry_level

    def throw_to(self, worry_level) -> int:
        if worry_level % self.modulo:
    ...
    with open('inputs/input_day11', 'r') as file:
        for line in file:
            ...
            if line.find('Starting items:') > -1:
                ...
                monkey_list[i].item_list = item_list
            if line.find('Operation:') > -1:
                operator, value = line.split(' ')[-2:]
                if operator == '*':
                    if value == 'old':
                        monkey_list[i].potency = 2
                    else:
                        monkey_list[i].worry_mult = int(value)
                else:
                    monkey_list[i].worry_add = int(value)
            if line.find('Test:') > -1:
                monkey_list[i].modulo = int(line.split(' ')[-1])
            if line.find('If true:') > -1:
                monkey_list[i].true_monkey = int(line.split(' ')[-1])
            if line.find('If false:') > -1:
                monkey_list[i].false_monkey = int(line.split(' ')[-1])
    return monkey_list


def part1():
    monkey_list = ConvertInput()
    for i in range(0, 20):
        for monkey in monkey_list:
            for item in monkey.item_list:
                new_worry_level = monkey.worry_level_after_inspection(item)
                monkey_list[monkey.throw_to(new_worry_level)].item_list.append(new_worry_level)
            monkey.item_list = []
    return monkey_list
...
def part2():
    ...
    combined_modulo = 1
    for monkey in monkey_list:
        combined_modulo *= monkey.modulo
    for i in range(0, 10000):
        for monkey in monkey_list:
            for item in monkey.item_list:
                new_worry_level = monkey.calculate_worry_level(item) % combined_modulo
                monkey_list[monkey.throw_to(new_worry_level)].item_list.append(new_worry_level)
            monkey.item_list = []
    return monkey_list
...
if __name__ == "__main__":
    ...
    no2 = 0
    for monk in monkeys:
        if monk.inspected_items > no1:
            no2 = no1
            no1 = monk.inspected_items
        elif monk.inspected_items > no2:
            no2 = monk.inspected_items
    print(no1 * no2)
