...
def read_start_stack_list(file):
    line = file.readline()
    stack_list = [[] for i in range(int((len(line) + 1) / 4))]
    while line[1] != '1':
        line = line.strip('\n')
        line = line[1::4]
        for i, crate in enumerate(line):
            if crate != ' ':
                stack_list[i].append(crate)
        line = file.readline()
...
def convert_instruction(string):
    string = re.findall(r'\d+', string)
    return int(string[0]), int(string[1])-1, int(string[2])-1
...
def part1():
    with open('input_day5', 'r') as file:
        ...
        while instruction != '':
            counter, stack_out, stack_in = convert_instruction(instruction)
            while counter != 0:
                crate = stack_list[stack_out].pop()
                stack_list[stack_in].append(crate)
                counter -= 1
            instruction = file.readline()
    code = ''
    for stack in stack_list:
...
def part2():
    with open('input_day5', 'r') as file:
        ...
        instruction = file.readline()
        while instruction != '':
            number_of_crates, stack_out, stack_in = convert_instruction(instruction)
            moving_stack = stack_list[stack_out][-number_of_crates:]
            stack_list[stack_out] = stack_list[stack_out][:-number_of_crates]
            stack_list[stack_in] += moving_stack
            instruction = file.readline()
    code = ''
    for stack in stack_list:
        code += stack.pop()
