...
Within the terminal output, lines that begin with $ are commands you executed, very much like some modern computers:
    ...
    on the argument: cd x moves in one level: it looks in the current directory for the directory named x and makes
    it the current directory. cd .. moves out one level: it finds the directory that contains the current directory,
...
class Directory:
    ...
    def update_size(self):
        new_size = 0
        for directory in self.sub_directories.items():
            new_size += directory[1]
        for size in self.files.items():
            new_size += size[1]
        self.size = new_size
        if self.parent is not None:
            self.parent.sub_directories[self.path] = new_size
...
class FileSystem:
    ...
    def __init__(self):
        self.directories_path_dict = dict()

    def directory_set_parent(self, directory: Directory):
        parent_path = directory.path[:directory.path.rfind('/')]
        if parent_path != '':
            directory.set_parent(self.directories_path_dict[parent_path])

    def add_directory(self, directory: Directory):
        self.directories_path_dict[directory.path] = directory
        self.directory_set_parent(directory)

    def directory_in_file_system(self, directory_path) -> bool:
        return directory_path in self.directories_path_dict

    def get_directory(self, path: str) -> Directory:
        return self.directories_path_dict[path]
...
def build_file_system() -> FileSystem:
    with open('input_day7', 'r') as file:
        ...
        current_path = '/root'
        current_directory = Directory(current_path)
        file_system.add_directory(current_directory)

        # Lese und Konvertiere Terminal-Output in ein File-System
        for terminal_output in file:
            terminal_output = terminal_output.strip('\n')
            if terminal_output[:4] == '$ cd':
                if terminal_output[5:7] == '..':
                    current_path = current_path[:current_path.rfind('/')]
                elif terminal_output[5] == '/':
                    current_path = '/root'
                else:
                    current_path += '/' + terminal_output[5:]
                current_directory = file_system.get_directory(current_path)
            elif terminal_output[0:4] == '$ ls':
                pass
            elif terminal_output[0].isdigit():
                size, file_name = terminal_output.split(' ')
                current_directory.add_file(file_name, int(size))
            elif terminal_output[:3] == 'dir':
                directory_path = current_path + '/' + terminal_output[4:]
                if not file_system.directory_in_file_system(directory_path):
...
def part1():
    ...
    directory_sum = 0
    for path, directory in file_system.directories_path_dict.items():
        if directory.size < 100000:
            directory_sum += directory.size
...
def part2():
    file_system = build_file_system()
    free_space = 70000000 - file_system.directories_path_dict['/root'].size
    needed_space = 30000000
    smallest_directory_to_delete_size = file_system.directories_path_dict['/root'].size
    for path, directory in file_system.directories_path_dict.items():
        if directory.size >= needed_space - free_space:
            smallest_directory_to_delete_size = min(directory.size, smallest_directory_to_delete_size)
    return smallest_directory_to_delete_size
