...
35390
    ...
    immediately; it can see only 1 tree (of height 5, right next to it). Looking right, its view is not blocked; it
    can see 2 trees. Looking down, its view is blocked eventually; it can see 2 trees (one of height 3, then the tree
...
def part1():
    ...
    with open('input_day8', 'r') as file:
        ...
        tree_matrix = np.matrix(tree_matrix)
    visible_matrix = np.zeros_like(tree_matrix)
    visible_matrix[0] = 1
    visible_matrix[-1] = 1
    visible_matrix[:, 0] = 1
    visible_matrix[:, -1] = 1
    for x in range(1, tree_matrix.shape[0] - 1):
        for y in range(1, tree_matrix.shape[1] - 1):
            if tree_matrix[x, y] > np.amax(tree_matrix[:x, y]) or tree_matrix[x, y] > np.amax(tree_matrix[x + 1:, y]) or \
                    tree_matrix[x, y] > np.amax(tree_matrix[x, :y]) or tree_matrix[x, y] > np.amax(
                tree_matrix[x, y + 1:]):
                visible_matrix[x, y] = 1
...
def part2():
    ...
    with open('input_day8', 'r') as file:
        for line in file:
            ...
            tree_matrix.append([int(height) for height in line])
        tree_matrix = np.matrix(tree_matrix)
    scenic_score_matrix = np.ones_like(tree_matrix)
    scenic_score_matrix[0] = 0
    scenic_score_matrix[-1] = 0
    scenic_score_matrix[:, 0] = 0
    scenic_score_matrix[:, -1] = 0
    for x in range(1, tree_matrix.shape[1] - 1):
        for y in range(1, tree_matrix.shape[0] - 1):
            distance = 1
            k = x
            while tree_matrix[x, y] > tree_matrix[k - 1, y] and k > 1:
                distance += 1
                k -= 1
            scenic_score_matrix[x, y] *= distance
            distance = 1
            k = x
            while tree_matrix[x, y] > tree_matrix[k + 1, y] and k < tree_matrix.shape[1] - 2:
                distance += 1
                k += 1
            scenic_score_matrix[x, y] *= distance
            distance = 1
            k = y
            while tree_matrix[x, y] > tree_matrix[x, k - 1] and k > 1:
                distance += 1
                k -= 1
            scenic_score_matrix[x, y] *= distance
            distance = 1
            k = y
            while tree_matrix[x, y] > tree_matrix[x, k + 1] and k < tree_matrix.shape[0] - 2:
                distance += 1
                k += 1
            scenic_score_matrix[x, y] *= distance
    return scenic_score_matrix.max()
