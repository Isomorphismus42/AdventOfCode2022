...
def move_tail(head_position, tail_position):
    ...
    if distance == 2:
        tail_position += (0.5 * difference).astype(int)
    elif distance > 2:
...
def create_movement_list():
    ...
    with open('input_day9', 'r') as file:
        for line in file:
            ...
            if line[0] == 'U':
                max_size[0] += stepsize
                movement_list.append((0, stepsize))
            elif line[0] == 'D':
                max_size[1] += stepsize
                movement_list.append((1, stepsize))
            elif line[0] == 'L':
                max_size[2] += stepsize
                movement_list.append((2, stepsize))
            elif line[0] == 'R':
                max_size[3] += stepsize
                movement_list.append((3, stepsize))
        return movement_list, max_size


def part1():
    movement_list, max_size = create_movement_list()
    visited_matrix = np.zeros((max_size[0] + max_size[1], max_size[2] + max_size[3]))
    head_position = np.array([max_size[0], max_size[2]])
    tail_position = np.array([max_size[0], max_size[2]])
    for movement in movement_list:
        ...
        while steps > 0:
            ...
            if movement[0] == 0:  # UP
                head_position[0] -= 1
            if movement[0] == 1:  # DOWN
                head_position[0] += 1
            if movement[0] == 2:  # LEFT
                head_position[1] -= 1
            if movement[0] == 3:  # RIGHT
                head_position[1] += 1
            tail_position = move_tail(head_position, tail_position)
            visited_matrix[tail_position[0], tail_position[1]] = 1
    return visited_matrix.sum()


def part2():
    movement_list, max_size = create_movement_list()
    tail_visited_matrix = np.zeros((max_size[0] + max_size[1], max_size[2] + max_size[3]))
    head_position = np.array([max_size[0], max_size[2]])
    knots_list = [np.array([max_size[0], max_size[2]]) for i in range(9)]
    knots_list.insert(0, head_position)
    for movement in movement_list:
        steps = movement[1]
        while steps > 0:
            steps -= 1
            ...
            if movement[0] == 3:  # RIGHT
                head_position[1] += 1
            for i in range(1, 10):
                knots_list[i] = move_tail(knots_list[i - 1], knots_list[i])
            tail_visited_matrix[knots_list[9][0], knots_list[9][1]] = 1
    return tail_visited_matrix.sum()
