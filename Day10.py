...
1. It supports only two instructions:
    ...
    negative.) noop takes one cycle to complete. It has no other effect.
...
Execution of this program proceeds as follows:
    ...
    first cycle, the noop instruction finishes execution, doing nothing. At the start of the second cycle,
    the addx 3 instruction begins execution. During the second cycle, X is still 1. During the third cycle,
    X is still 1. After the third cycle, the addx 3 instruction finishes execution, setting X to 4. At the start of
    the fourth cycle, the addx -5 instruction begins execution. During the fourth cycle, X is still 4. During the
    fifth cycle, X is still 4. After the fifth cycle, the addx -5 instruction finishes execution, setting X to -1.
...
The interesting signal strengths can be determined as follows:
    ...
    other addx values up to that point: 1 + 15 - 11 + 6 - 3 + 5 - 1 - 8 + 13 + 4 = 21.) During the 60th cycle,
    register X has the value 19, so the signal strength is 60 * 19 = 1140. During the 100th cycle, register X has the
    value 18, so the signal strength is 100 * 18 = 1800. During the 140th cycle, register X has the value 21,
    so the signal strength is 140 * 21 = 2940. During the 180th cycle, register X has the value 16, so the signal
    strength is 180 * 16 = 2880. During the 220th cycle, register X has the value 18, so the signal strength is 220 *
    18 = 3960.
...
def part1():
    ...
    answer = [i*signal_strengths[i] for i in range(20, 221, 40)]
    return sum(answer)
...
def part2():
    sprite = []
    signal_strengths = SignalStrengths()
    line = ['.' for j in range(40)]
    for i in signal_strengths:
        if i % 40 == 0:
            sprite.append(line)
            line = ['.' for j in range(40)]
        if abs(((i-1) % 40) - signal_strengths[i]) < 2:
            line[(i-1) % 40] = '#'
    for line in sprite:
        print(''.join(line))
